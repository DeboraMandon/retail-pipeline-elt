...
Mode automatique : PostgreSQL si Docker disponible, CSV sinon
...
engine = get_engine()
if engine is None:
    st.sidebar.warning("⚠️ Mode demo — donnees statiques")
    st.sidebar.caption("Pour les donnees en temps reel, lancez Docker : `docker compose up -d`")
else:
    st.sidebar.success("✅ Connecte a PostgreSQL")
    st.sidebar.caption("Donnees en temps reel depuis la base")
...
with tab1:
    ...
    with col1:
        ...
        C'est plus performant et tracable qu'un ETL classique.
        """)
...
with tab2:
    ...
    if engine is not None:
        ...
        df_raw_csv = load("raw_counts", engine, "")
        raw_data = []
        for table in raw_tables:
            row = df_raw_csv[df_raw_csv["table_name"] == table] if not df_raw_csv.empty else pd.DataFrame()
            if not row.empty:
                nb = int(row.iloc[0]["nb_lignes"])
                raw_data.append({"Table": f"raw.{table}", "Lignes": f"{nb:,}", "Statut": "✅ (demo)"})
            else:
                raw_data.append({"Table": f"raw.{table}", "Lignes": "-", "Statut": "📊 demo"})
    ...
    mart_data = []
    for table, default_count in mart_tables:
        if engine is not None:
            try:
            ...
            mart_data.append({"Modele": f"marts.{table}", "Lignes": default_count, "Statut": "✅ (demo)"})
    ...
    ml_tables = [
        ("forecast_revenue", "26"),
        ("satisfaction_predictions", "95 829"),
    ]
    ml_data = []
    for table, default_count in ml_tables:
        if engine is not None:
            try:
            ...
            ml_data.append({"Table": f"ml.{table}", "Lignes": default_count, "Statut": "✅ (demo)"})
...
with tab3:
    ...
    with col1:
        ...
        df_cat = load("top_categories", engine, """
    ...
    with col2:
        ...
        df_sat = load("satisfaction", engine, """
            SELECT date_trunc('month', order_purchased_at) AS mois,
    ...
    df_delai = load("delai_livraison", engine, """
...
with tab4:
    ...
    with col2:
        ...
        df_proba = load("satisfaction_proba", engine, """
    ...
    if engine is not None:
        ...
        st.info("🔍 L'exploration interactive des predictions necessite une connexion PostgreSQL (docker compose up -d)")